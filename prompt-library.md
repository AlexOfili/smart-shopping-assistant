...
**Why it works:** the weak version can't be trusted or automated — nothing stops it suggesting an out-of-stock item or replying in free text a program can't parse. The strong version is grounded (can't recommend anything outside the list) and machine-readable (the total can be extracted and checked against budget automatically).
...
**Why it works:** constraining the model to a fixed, closed list of allowed answers (**Constraints** + **Format**) means the output is always one of six valid strings — safe to insert straight into the `PRODUCTS` list without extra validation logic.
...
**Why it works:** a tight length limit and an explicit "do not repeat the price" rule (**Constraints** + **Format**) stop the model padding its answer — the output is short enough to drop straight into a UI tooltip without truncation or overflow.
